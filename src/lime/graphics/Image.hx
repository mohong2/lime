...
class Image
{
	...
	**/
			...
			mergeAlpha:Bool = false):Void
	{
		...
		{
			// Self-overlapping copy: use row-buffer to avoid full image clone
			// This saves significant memory (e.g. 1920*1080*4 = ~8MB per clone)
			var isDownward:Bool = (destPoint.y > sourceRect.y);

			switch (type)
			{
				case CANVAS:
					ImageCanvasUtil.convertToCanvas(this);
					ImageCanvasUtil.copyPixels(this, this, sourceRect, destPoint, null, null, mergeAlpha);

				case DATA:
					#if (js && html5)
					ImageCanvasUtil.convertToData(this);
					#end
					ImageDataUtil.copyPixelsSelf(this, sourceRect, destPoint, isDownward);

				case FLASH:
					sourceRect.offset(offsetX, offsetY);
					destPoint.offset(offsetX, offsetY);
					buffer.__srcBitmapData.copyPixels(buffer.__srcBitmapData,
						sourceRect.__toFlashRectangle(), destPoint.__toFlashPoint(),
						null, null, mergeAlpha);
	...
	**/
	...
	/**
	 * Releases the pixel data held by this Image's buffer.
	 * After calling this, the Image should no longer be used for
	 * rendering. Useful for freeing memory when an asset is no longer needed.
	 */
	...
	/**
	 * Release only the raw CPU-side pixel data (buffer.data),
	 * preserving other buffer state.
	 *
	 * Use after uploading to GPU to reclaim CPU memory — critical
	 * on mobile (unified memory architecture) where keeping both
	 * CPU and GPU copies doubles memory usage.
	 */
	...
	/**
	 * Check if raw pixel data has been released.
	 */
	public var dataReleased(get, never):Bool;

	@:noCompletion private function get_dataReleased():Bool
	{
		return buffer == null || buffer.data == null;
	}

	/**
	 * Cache of pre-scaled versions of this image.
	 * Key = scale factor rounded to 4 decimal places, Value = scaled Image.
	 *
	 * Useful on mobile where you may render the same sprite at
	 * different sizes frequently — avoids repeated resize() calls.
	 * Call `clearScaledVersions()` to free this memory.
	 */
	public var scaledVersions:Map<String, Image> = new Map<String, Image>();

	/**
	 * Get or create a pre-scaled version of this image.
	 * Subsequent calls with the same scale return the cached copy.
	 *
	 * @param scale Scale factor (0.5 = half size, 1.0 = original)
	 * @return A scaled Image (cached) or null if scale is invalid
	 */
	public function getScaledVersion(scale:Float):Image
	{
		if (scale <= 0 || scale == 1.0) return this;
		var key:String = haxe.format.JsonPrinter.print(scale);
		if (scaledVersions.exists(key)) return scaledVersions.get(key);

		var newWidth:Int = Std.int(Math.ceil(width * scale));
		var newHeight:Int = Std.int(Math.ceil(height * scale));
		if (newWidth < 1 || newHeight < 1) return null;

		var scaled:Image = clone();
		scaled.resize(newWidth, newHeight);
		scaledVersions.set(key, scaled);
		return scaled;
	}

	/**
	 * Release all cached scaled versions of this image.
	 * Frees memory without affecting the original.
	 */
	public function clearScaledVersions():Void
	{
		for (img in scaledVersions)
		{
			if (img != null) img.dispose();
		}
		scaledVersions = new Map<String, Image>();
	}
}
