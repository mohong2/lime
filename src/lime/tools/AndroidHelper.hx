...
class AndroidHelper
{
	...
	public static function build(project:HXProject, projectDirectory:String):Void
	{
		...
		{
			// Windows: CreateProcess cannot run .bat directly; it must be
			// wrapped in cmd.exe /c, and the batch must be referenced by an
			// absolute path (cmd /c does not reliably resolve a bare relative
			// name here). The working directory is projectDirectory.
			var gradlewPath = Path.combine(projectDirectory, "gradlew.bat");
			// projectDirectory may be relative (e.g. "export/release/android/bin");
			// cmd.exe /c would then treat the first path segment ("export") as a
			// command. Make it absolute before handing it to cmd.
			if (gradlewPath.indexOf(Sys.getCwd()) != 0)
			{
				gradlewPath = Path.combine(Sys.getCwd(), gradlewPath);
			}
			System.runCommand(projectDirectory, "cmd.exe", ["/c", gradlewPath].concat(args));
		}
	}
