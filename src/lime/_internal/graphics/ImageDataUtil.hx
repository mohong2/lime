...
class ImageDataUtil
{
	// Row buffer pool for copyPixelsSelf — avoids per-call allocation at high FPS.
	// Grows on demand but never shrinks, amortizing cost over the app lifetime.
	#if js
	private static var __pooledRowBuffer:lime.utils.UInt8Array = null;
	#else
	private static var __pooledRowBuffer:lime.utils.UInt8Array = null;
	private static var __pooledRowBufferBytes:haxe.io.Bytes = null;
	#end
	private static var __pooledRowBufferSize:Int = 0;
	...
	/**
	 * Optimized self-overlap copy: uses a single row buffer instead of
	 * cloning the entire image. `copyDownward`: true if dest is below source.
	 */
	public static function copyPixelsSelf(image:Image, sourceRect:Rectangle, destPoint:Vector2, copyDownward:Bool):Void
	{
		var data = image.buffer.data;
		if (data == null) return;

		var bpp:Int = Std.int(image.buffer.bitsPerPixel / 8);
		var stride:Int = image.buffer.width * bpp;
		var rowBytes:Int = Std.int(sourceRect.width * bpp);

		if (rowBytes <= 0) return;

		// Rectangle fields are Float, cast to Int for offset math
		var srcX:Int = Std.int(sourceRect.x);
		var srcY:Int = Std.int(sourceRect.y);
		var dstX:Int = Std.int(destPoint.x);
		var dstY:Int = Std.int(destPoint.y);
		var rows:Int = Std.int(sourceRect.height);

		// Use pooled row buffer — grow only when needed, avoid per-call allocation
		if (rowBytes > __pooledRowBufferSize)
		{
			__pooledRowBuffer = new UInt8Array(rowBytes);
			#if !js
			__pooledRowBufferBytes = __pooledRowBuffer.buffer;
			#end
			__pooledRowBufferSize = rowBytes;
		}

		#if js
		var rowBuffer = __pooledRowBuffer;
		#else
		var rbBuffer = __pooledRowBufferBytes;
		var rbOffset:Int = Std.int(__pooledRowBuffer.byteOffset);
		#end
		...
		if (copyDownward)
		{
			...
			while (y >= 0)
			{
				var srcOffset:Int = (srcY + y) * stride + srcX * bpp;
				var dstOffset:Int = (dstY + y) * stride + dstX * bpp;

				#if js
				rowBuffer.set(data.subarray(srcOffset, srcOffset + rowBytes));
				data.set(rowBuffer, dstOffset);
				#else
				rbBuffer.blit(rbOffset, data.buffer, srcOffset, rowBytes);
				data.buffer.blit(dstOffset, rbBuffer, rbOffset, rowBytes);
				#end
	...
	public static function fillRect(image:Image, rect:Rectangle, color:Int, format:PixelFormat):Void
	{
		...
		#end
		{
			var imgFormat = image.buffer.format;
			var premultiplied = image.buffer.premultiplied;
			...
			if (bpp <= 0) bpp = 4;
			var stride:Int = image.buffer.width * bpp;
			// Rectangle fields are Float — cast to Int
			...
			var rowBytes:Int = rectW * bpp;

			if (rowBytes <= 0) return;

			var startOffset:Int = rectY * stride + rectX * bpp;

			// Optimized: write first row as pattern, then bulk-copy to all rows.
			// This replaces O(w*h) per-pixel writes with O(w + h) bulk copies.
			if (imgFormat == RGBA32 && bpp == 4 && !premultiplied)
			{
				// Fast path: pure RGBA — pattern is exactly 4 bytes, repeatable
				var r:Int = fillColor.r;
				var g:Int = fillColor.g;
				var b:Int = fillColor.b;
				var a:Int = fillColor.a;

				#if js
				// Build first row by setting 4-byte chunks
				var rowOff:Int = startOffset;
				for (x in 0...rectW)
				{
					data[rowOff] = r;
					data[rowOff + 1] = g;
				...
				// Bulk-copy first row to remaining rows
				for (y in 1...rectH)
				{
					data.set(data.subarray(startOffset, startOffset + rowBytes), startOffset + y * stride);
				}
				#else
				// Native: use blit for bulk row copies
				var rowOff:Int = startOffset;
				for (x in 0...rectW)
				{
					data.buffer.set(rowOff, r);
					data.buffer.set(rowOff + 1, g);
					data.buffer.set(rowOff + 2, b);
					data.buffer.set(rowOff + 3, a);
					rowOff += 4;
				}
				for (y in 1...rectH)
				{
					data.buffer.blit(startOffset + y * stride, data.buffer, startOffset, rowBytes);
				}
				#end
			}
			else
			{
				// Generic path: per-pixel write for first row, then bulk copy
				#if js
				for (x in 0...rectW)
				{
					fillColor.writeUInt8(data, startOffset + x * bpp, imgFormat, false);
				}
				for (y in 1...rectH)
				{
					data.set(data.subarray(startOffset, startOffset + rowBytes), startOffset + y * stride);
