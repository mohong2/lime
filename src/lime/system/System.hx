...
class System
{
	...
	/**
	 * Whether the current platform is a mobile device (Android, iOS, tvOS).
	 * Useful for adjusting memory limits, texture sizes, and quality settings.
	 *
	 * Determined at compile-time, so there is zero runtime overhead.
	 */
	...
	private static function get_isMobile():Bool
	{
		#if (android || ios || tvos || tizen || webos)
		return true;
		#else
		return false;
		#end
